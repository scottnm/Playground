...
$devkitadvPathUpdate = ';C:\devkitadv-r5-beta-3\bin'
try {
    # first setup the environment so that we can reach all of the necessary
    $env:PATH += $devkitadvPathUpdate
    Run-Build
} finally {
    $env:PATH = ($env:PATH.Replace($devkitadvPathUpdate,""))
}
