...
def selection(__seq : List[T], *, \
    ...
    for unsorted_mark in range(0, len(__seq) - 1):
        ...
        for i in range(unsorted_mark + 1, len(__seq)):
            if comp_gt(seq[min_index], seq[i]):
                min_index = i
    ...
    while len(sublist_stack) > 0:
        ...
        if ele_le > ele_gt:
            ...
            while gt_swaps > 0 and le_runner < ele_le:
                if comp_gt(seq[left_0 + le_runner], pivot_ele):
                    while True:
                        if not comp_gt(seq[right_0 + gt_runner], pivot_ele):
                            break
                        else:
                            gt_runner += 1
